...
public class BitcoinJsonRpcImpl implements  BitcoinJsonRpc{
  ...
  public BitcoinJsonRpcImpl(@Value("${bitcoin.jsonrpc.url}") String url,
                            @Value("${bitcoin.jsonrpc.username}")String name,
                            @Value("${bitcoin.jsonrpc.password}") String pwd) throws MalformedURLException {
      jsonRpcHttpClient = new JsonRpcHttpClient(new URL(url));
      HashMap<String, String> headers = new HashMap<String, String>();
      String string=name+":"+pwd;
      String string64= Base64.getEncoder().encodeToString(string.getBytes());
              headers.put("Authorization","Basic "+string64);
